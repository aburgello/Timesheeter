...
export default function ExcalidrawPageEditor({ pageId, content, onChange }) {
  ...
  return (
    // Excalidraw does its own screen-to-canvas coordinate math internally and
    // has no idea whether an ancestor is scaling the page, so it needs to sit
    // at a net zoom of exactly 1. This used to mean an inner layer applying
    // the inverse (1/1.1) of the app-wide `zoom: 1.1`, with 110% width/height
    // to undo that layer's own visual shrink. The app-wide zoom is gone, so
    // the subtree is already at 1 and the inner layer just fills its parent.
